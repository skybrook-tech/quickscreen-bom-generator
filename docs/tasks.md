...
Latest Brief 010 pass: the canvas Map off/on control now hides both captured map layers, restores the previous layer visibility and opacity state when turned back on, and removes the inert standalone Layers icon button treatment from the toolbar.
