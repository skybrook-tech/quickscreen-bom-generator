-- ============================================================================
-- 032_backfill_glass_outlet_supplier_and_instances.sql
-- (ported from fork migration 033, renumbered for main — salvage Phase A)
--
...
--
-- Note (salvage Phase A deviation): the Glass Outlet supplier row is linked to
-- the glass-outlet organisation via org_id. The fork left suppliers.org_id NULL
-- for platform suppliers; we set it so price-book RLS can scope by org without a
-- separate mapping table. See 037_multi_supplier_rls.sql.
-- ============================================================================
...
-- Link the Glass Outlet supplier to the glass-outlet organisation (salvage Phase A
-- deviation from the fork). This lets price-book RLS scope by org_id via
-- suppliers.org_id without trusting client input.
UPDATE suppliers s
   SET org_id = o.id
  FROM organisations o
 WHERE s.slug = 'glass-outlet'
   AND o.slug = 'glass-outlet'
