// engine_test_fixtures.ts — TEST-ONLY frozen catalogue + pricing fixture.
//
// This is the former engine.ts "synthetic" catalogue, moved out verbatim so the
// RUNTIME engine is DB-only (offline/no-DB runs produce correct SKUs+quantities
// but $0 prices — this is an online app; the DB is the only price source).
// The engine_test.ts snapshot suite builds an explicit CalcContext from these
// arrays via makeCalcContext(), keeping the price-bearing snapshots stable and
// the tier-pricing tests meaningful. Do NOT import this from runtime code.
