...
Latest Brief 009 pass: canvas section, length, angle, and post labels now render as smaller pill labels with consistent light backgrounds, drop shadows, reduced angle clutter, and a one-pass overlap adjustment while keeping the underlying label data unchanged.
