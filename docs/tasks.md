...
- [x] Brief BC unified calculator experience: entry methods now use the same workspace state, Draw opens the Map tab while Describe/Select open BOM, the sticky map overlay was replaced by persistent Map/BOM tabs, typed/canvas length edits continue to sync through the canonical payload, gate openings are attributed to parent sections with inline section chips/Add Gate actions, expanded map mode fills the workspace to the right of the sidebar, map shortcuts/help were surfaced, and run details now render below the map while preserving the flat BOM-compatible `gate_opening` segments.
