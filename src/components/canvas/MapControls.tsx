...
export function MapControls({ engineRef }: MapControlsProps) {
  ...
  const handleLoadMap = async () => {
    ...
    try {
      ...
      const key = (window as unknown as Record<string, string>)['GOOGLE_MAPS_KEY'] ?? '';
      if (!key) {
        setError('Google Maps API key not configured. Set window.GOOGLE_MAPS_KEY or add a script tag.');
        setLoading(false);
        return;
      }
  ...
  return (
    <div className="flex flex-wrap items-start gap-4 p-3 bg-brand-card border border-t-0 border-brand-border rounded-b text-xs">
      {/* Address search */}
      ...
      <div className="flex items-center gap-2">
        ...
        />
        <span className="text-brand-muted w-8">{Math.round(opacity * 100)}%</span>
      </div>
