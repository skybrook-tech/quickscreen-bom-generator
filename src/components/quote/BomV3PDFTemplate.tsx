...
interface BomV3PDFTemplateProps {
  ...
  siteAddress?: string;
  validUntil?: string;
  /** Current org's display name — replaces the hardcoded "The Glass Outlet". */
  orgName?: string | null;
  /** Current org's logo image URL, rendered in the header when present. */
  logoUrl?: string | null;
}
...
export function BomV3PDFTemplate({
  ...
  const orgLabel = orgName?.trim() || "QuickScreen BOM";
  ...
  return (
    <Document>
      <Page size="A4" style={s.page}>
        ...
        <View style={s.header}>
          <View style={s.brandRow}>
            ...
            <View>
              ...
              <Text style={s.brandSecond}>{orgLabel} — QuickScreen BOM</Text>
            </View>
          </View>
        ...
        {/* Footer */}
        <Text style={s.footer}>
          Generated by SkybrookAI · {orgLabel} · All prices ex-GST unless noted
        </Text>
      </Page>
    </Document>
  );
