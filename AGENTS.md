...
6. **New system code = a migration**: extend the `chk_system_types_values` CHECK constraint on `product_components` (see `002_timber_paling_system_type.sql` for the pattern) AND the matching enum in `supabase/seeds/schemas/product_components.schema.json` — the seeder validates against the schema before the DB does.
