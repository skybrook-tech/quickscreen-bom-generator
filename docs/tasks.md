...
Latest pricing-rules slim: sparse tiers (2026-07-09, follow-up on "why 18k pricing rules?"): analysis showed 97% of rule volume was **tier duplication with no behavioural effect** — of 6,154 distinct `(sku, rule, priority)` groups only 47 had any tier1/2/3 price difference, and the engine's qty-gated path (98.2% of rules) **ignored `tier_code` entirely** (`priceForSku` matched across all tiers, so tier2/3 rows were never consulted — trade tiers silently got tier1 prices where tiers DID differ). Changes: **(1) engine** — `priceForSku`'s gated path is now tier-aware: overlay the requested tier's rows onto the tier1 base per `(rule, priority)` break, with a legacy any-tier last resort (never $0 on weird data). **(2) seeds slimmed** — kept tier1 + only real tier2/3 overrides: 18,570 → 6,342 rules (−65.8%); AF timber's genuine trade tiers untouched. **(3) importers fixed** — `import-glassoutlet-prices.mjs` + `build-brief-at-pricing-seed.mjs` emit tier1 only (they were the triplication source). **(4) new `tools/prune-duplicate-tier-rules.js`** — deletes tier2/3 rows exactly duplicating tier1 (same component/rule/priority/price) from pre-slim DBs (upsert never deletes); dry-run default, `--apply` to delete; differing-price rows (real overrides, live customer edits) never touched; verified via planted-dupe test. **Verified by exhaustive price parity**: 85,560 (org × sku × qty × tier) price points computed with old-engine+old-data vs new-engine+slimmed-data — **tier1: 0 diffs** (every current user identical); the only 520 diffs are tier2/3 on 26 AF timber SKUs now correctly getting their documented trade prices instead of tier1 retail (the latent bug, fixed). 95 unit + 7 DB integration tests + build green; snapshots byte-identical. Docs: seeds/README "Pricing tiers are SPARSE" section, AGENTS.md §5 pricing_rules row.

Seed-products remote perf + idempotency fix (2026-07-09): remote `seed:products` appeared to hang on glass-outlet `price_catalogue.json` (14,808 pricing rules). Root cause: `upsertPricingRules` did per-row **check-then-insert/update** = ~2 sequential HTTP round trips per rule (~30k requests) — fine at ~1ms local latency (~60s), pathological at ~150–300ms remote (~100 min). Rewrote it to **batch**: `bulkResolveComponentIds` (chunked `.in()`), a paginated bulk-fetch of existing rules keyed by `(component_id|tier_code|priority|active)`, then a bulk `.insert()` (new) + bulk `.upsert(onConflict:'id')` (existing) in 500-row chunks — ~30k round trips → a few dozen; full local seed ~2s. Two bugs caught in local verification before they hit prod: (a) the bulk existing-rules read hit **PostgREST's 1000-row/response cap** (200 components × ~6.6 rules > 1000), silently truncating and mis-classifying rules as inserts → `duplicate key` on the partial unique index; fixed by paginating each chunk with `.range()`. (b) a pre-existing **idempotency leak**: 9 `active:false` qs_gate rules were re-inserted every run (the unique index is partial `WHERE active=true`, so inactive rows are unconstrained and never matched) — fixed by including `active` in the match key and fetching both states. Verified idempotent from a clean reset: re-run reports `0 inserted` for every file, active=18540 / inactive=9 stable across runs, floors pass, price spot-check matches. File: `supabase/seeds/tools/seed-products.js` (`resolveComponentId`/component-id cache removed, replaced by `bulkResolveComponentIds`).

Migrations → timestamp naming (2026-07-09): switched `supabase/migrations/` from sequential `001_/002_` to the Supabase CLI default `YYYYMMDDHHMMSS_name.sql` (`001_init.sql` → `20260706000000_init.sql`, `002_timber_paling_system_type.sql` → `20260708000000_timber_paling_system_type.sql`; timestamps track the real squash/commit dates). Avoids the `003_` collision two branches hit adding "the next" migration, and matches `supabase migration new`. Local `db reset` replays the renamed files unchanged (14-digit prefixes sort lexicographically). Doc refs updated (AGENTS.md §3/§5/§11a, both migration headers, `product_components.schema.json` comment, tasks.md current-phase). **Remote requires a one-time history repair** because Supabase tracks applied migrations by the filename version prefix: the remote's `schema_migrations` row is `001`, so after the rename you must `supabase migration repair --status reverted 001 --linked` + `--status applied 20260706000000 --linked` (repoints init, no SQL re-run), then `supabase db push` applies the still-missing timber migration (`20260708000000…`). Verify with `supabase migration list --linked`. (The remote was also missing the timber migration independently — it had only the init applied.)
