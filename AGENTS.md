...
The schema lives in a **single squashed migration** (`supabase/migrations/20260706000000_init.sql`, 2026-07 compaction — the former 001–032 numbering survives only in git history). Nine tables + one view:

| Table / View                | Key design notes                                                                                                                                    |
| --------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------- |
| `organisations`             | Tenant orgs (name/slug/branding JSONB incl. `logo_url`) seeded from per-org `seeds/<slug>/org.json` via `npm run seed:orgs`. No RLS.                    |
...
| `products`                  | FLAT (no parent/variant hierarchy); `product_type` distinguishes fence/gate/other; `image_url` via `seed-images.js`. RLS+grant: `authenticated` SELECT (metadata only). `chk_system_types_values` on `product_components` constrains allowed codes (currently QSHS/VS/XPL/BAYG/GATE/COLORBOND/TIMBER_PALING — extending it = new migration + the seed-schema enum, see § 11a). |
...
| `pricing_rules`             | **No RLS** (same grant posture). `component_id` FK, `tier_code` (tier1/2/3 — **sparse**: tier1 holds the full schedule, tier2/3 rows exist only where that tier's price differs; the engine overlays per break at price time), `rule` (only `qty >= N`-style quantity gates are evaluated — see engine `matchesPriceRule`), `price`, `priority`. |
...
⚠️ When adding migrations on top of the init migration: Supabase's `ALTER DEFAULT PRIVILEGES` grants ALL to `anon`/`authenticated` on every new table — tables holding pricing IP without RLS must explicitly REVOKE (see the ACL block at the end of `20260706000000_init.sql`), and anything on the `auth` schema (triggers) is never captured by `supabase db dump`.
...
5. `strategy.fence` union member in `config/types.ts` AND its client mirror `src/types/calculatorConfig.types.ts` if the calculation strategy is new.
6. **New system code = a migration**: extend the `chk_system_types_values` CHECK constraint on `product_components` (see `20260708000000_timber_paling_system_type.sql` for the pattern) AND the matching enum in `supabase/seeds/schemas/product_components.schema.json` — the seeder validates against the schema before the DB does.
7. Components + pricing are **seed data**, not engine code: add rows to the owning org's seed JSON (`supabase/seeds/<org-slug>/products/<code>.json`) and reseed. Per-org rule differences go in the same file's `calculator_configs` section (sparse `CalculatorConfig` patches upserted into `supplier_product_calculator_configs`; arrays REPLACE, objects merge).
...
**Gates** are separate fields-only products paired to fences via `gateRules.gateProductCode` + `compatible_with_system_types` (`QS_GATE` for the slat systems, `CB_GATE` for Colorbond). A new slat-like fence pairs with the existing `QS_GATE`; a fence whose gates are built differently gets its own gate block on its strategy config (see `ColorbondConfig.gates` — `kit` vs `bundle` modes) and a fields-only `BASE_<X>_GATE_CONFIG`.
...
- **The live calculator is the static engine, not the DB-driven one.** To change calculation behaviour, edit the config under `supabase/functions/bom-calculator-static/config/` (see § 5a) — the DB rule tables no longer exist (dropped in the 2026-07 compaction). The `supabase/seeds/<org-slug>/products/*.json` files contain only LIVE sections (`products` / `product_components` / `pricing_rules` / `calculator_configs` — the engine prices from these DB rows and deep-merges the overlay patches): edit + `npm run seed:products` to change catalogue/pricing facts or per-org rule values.
