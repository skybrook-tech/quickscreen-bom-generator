...
export function RightPaneTabs({ activeView, onChange }: RightPaneTabsProps) {
  return (
    <div className=" border-brand-primary/35 bg-brand-bg/70 p-1 shadow-inner">
      {tabs.map(({ id, label, icon: Icon }) => (
        <button
          key={id}
          type="button"
          onClick={() => onChange(id)}
          className={`inline-flex min-w-[5.5rem] items-center justify-center gap-2 rounded-lg px-4 py-1 text-sm font-black transition-colors ${activeView === id
