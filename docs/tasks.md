...
Latest Brief BQ pass: the map no longer shows the first-draw hint after the first fence click until the canvas is cleared, the old fence-line hover stats tooltip was removed, and Gate mode now opens a persistent draggable Edit Gate dialog that previews settings while hovering a fence line, supports repeated placements, keeps placed gates synced with dialog changes, and can cancel the current placement session.
