...
from concrete_helpers import (
    CONCRETE_TARGET_PER_POST_KG,
    DEFAULT_BAG_SIZE_KG,
    DEFAULT_PRODUCT_TYPE,
    ALLOWED_BAG_SIZES_KG,
    ALLOWED_PRODUCT_TYPES,
    bags_per_post,
    total_concrete_bags,
    canonical_concrete_name,
)
...
# Concrete now parameterised — see concrete_helpers.py.
# Target: 30kg of concrete per post (industry standard for ~300×600mm hole).
# Bag size is a per-call input (`concrete_bag_size_kg`), defaults to 20.
# Old constants CONCRETE_BAGS_PER_POST and CONCRETE_BAG_SIZE_KG removed —
# their values are now derived: bags_per_post(20) = 1.5, bag_size = per-call.
...
def validate_inputs(inputs: dict) -> tuple[list[str], list[str]]:
    ...
    # Concrete bag size (must be in the canonical contract)
    bag_size = inputs.get("concrete_bag_size_kg", DEFAULT_BAG_SIZE_KG)
    if bag_size not in ALLOWED_BAG_SIZES_KG:
        errors.append(
            f"concrete_bag_size_kg must be one of {ALLOWED_BAG_SIZES_KG}, "
            f"got {bag_size!r}. Adding a new size requires a canonical-name contract version bump."
        )

    # Concrete product type
    product_type = inputs.get("concrete_product_type", DEFAULT_PRODUCT_TYPE)
...
def calculate_bom(inputs: dict) -> dict:
    ...
    concrete_bag_size_kg = inputs.get("concrete_bag_size_kg", DEFAULT_BAG_SIZE_KG)
    concrete_product_type = inputs.get("concrete_product_type", DEFAULT_PRODUCT_TYPE)
    ...
    concrete_bags       = total_concrete_bags(post_count, concrete_bag_size_kg)
    concrete_bpp        = bags_per_post(concrete_bag_size_kg)  # for notes / derived output
    ...
    if paling_style == "butted":
        ...
        bom = [
            ...
            },
            {
                "line": 4,
                "category": "Concrete",
                "canonical_name": canonical_concrete_name(concrete_bag_size_kg, concrete_product_type),
                "description": canonical_concrete_name(concrete_bag_size_kg, concrete_product_type),
                "qty": concrete_bags,
                "unit": "bag",
                "notes": (
                    f"{concrete_bpp:g} {'bag' if concrete_bpp == 1.0 else 'bags'}/post "
                    f"({CONCRETE_TARGET_PER_POST_KG}kg target × {concrete_bag_size_kg}kg bags)"
                ),
            },
            {
                "line": 5,
    ...
    # Add derived concrete bag count to the derived block (post-style-branch).
    derived["concrete_bags_per_post"] = concrete_bpp
    ...
    return {
        ...
        "inputs": {
            ...
            "concrete_product_type":  concrete_product_type,
        },
        "derived": derived,
        "bom": bom,
        "warnings": warnings,
        "constants": {
            ...
            "CONCRETE_TARGET_PER_POST_KG": CONCRETE_TARGET_PER_POST_KG,
