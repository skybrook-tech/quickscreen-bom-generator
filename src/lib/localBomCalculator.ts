...
function normalizeColour(colour: string): string {
  ...
  if (STANDARD_COLOURS.has(clean.toUpperCase()) || ALUMAWOOD_CORE_COLOURS.has(clean.toUpperCase())) {
...
function calculateGateSegment(
  ...
  const colour = normalizeColour(String(vars[GATE_SEGMENT_STUB_KEYS.colourCode] ?? vars.colour_code ?? "B"));
...
function calculateVerticalSlatRun(
  ...
  const runColour = normalizeColour(String(mergedRunVars.colour_code ?? mergedRunVars.colour ?? "B"));
  const runPostColour = normalizeColour(String(mergedRunVars.post_colour_code ?? runColour));
